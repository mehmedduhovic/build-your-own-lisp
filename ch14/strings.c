...
void lval_print_str(lval* v) {
	char* content = malloc(strlen(v->str) + 1);
	strcpy(content, v->str):
	content = mpcf_escape(content);
	printf("\"%s\"", content);
	free(content);
}
...
lval* builtin_eval(lenv* e, lval* a) {
	LASSERT(a, a->count == 1, 
			"Error 'eval': Too many arguments. Expected %i, got %i", 1, a->count);
	LASSERT(a, a->cell[0]->type == LVAL_QEXPR, 
			"Error 'eval': Incorrect Type passed. Expected %s, got %s", 
...
lval* builtin_ord(lenv* e, lval* a, char* op) {
	LASSERT_TYPE(a, a->cell[0]->type, LVAL_NUM, "op");
...
lval* builtin_lambda(lenv* e, lval* a) {
	...
	for(int i = 0; i < a->cell[0]->count; i++) {
		LASSERT_TYPE(a->cell[0], a->cell[0]->cell[i]->type, LVAL_SYM, "\\");
	}
...
lval* lval_eval_sexpr(lenv* e, lval* v) {
	...
	lval* f = lval_pop(v, 0);
	if(f->type != LVAL_FUN) {
		lval* err = lval_err("First element is not a function. Got %s, expected %s",
...
lval* lval_read_str(mpc_ast_t* t) {
	t->contents[str(t->contents)-1] = '\0';
	char* x = malloc(strlen(t->contents+1)+1);
	strcpy(x, t->contents+1);
}
...
lval* lenv_get(lenv* e, lval* k) {
	...
	if(e->par) {
		...
		return lval_err("Symbol not found! '%s'", k->sym);
	}
...
lval* builtin_var(lenv* e, lval* a, char* func) {
	LASSERT(a, a->cell[0]->type == LVAL_QEXPR, "Error 'def': Incorrect type!");

	lval* syms = a->cell[0];

	for(int i = 0; i < syms->count; i++) {
		LASSERT(a, syms->cell[i]->type == LVAL_SYM, "Error 'def': Non-symbol provided");
	}

	LASSERT(a, (syms->count == a->count-1), "Error 'def': Incorrect number of values to symbols");
